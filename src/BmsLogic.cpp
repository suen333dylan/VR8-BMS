...
void BmsLogic::printOpenWireStatus(const BmsState& state, uint8_t ic) {
    ...
    if (!state.open_wire_valid[ic]) {
        Serial.print(F("pending"));
        return;
    }
    if (!hasOpenWireFault(state, ic)) {
        Serial.print(F("none"));
        return;
    }
    Serial.print(F("C"));
...
void BmsLogic::printCycleReport(const BmsState& state) {
    Serial.println(F("\n================================================================================"));
    Serial.print(F(" BMS Uptime: "));
    Serial.print(millis() / 1000);
    Serial.print(F("s | "));
    Serial.print(F("BMS_OK: "));
    Serial.print(state.bms_ok ? F("YES") : F("NO "));
    Serial.print(F(" | "));
    Serial.print(F("CAN: "));
    Serial.print(state.can_ready ? F("OK  ") : F("FAIL"));
    Serial.print(F(" | "));
    Serial.print(F("Discharge: "));
    Serial.print(state.discharge_enabled ? F("ON ") : F("OFF"));
    Serial.println(F("\n------------------------------------------------------------------------------------------"));
    ...
    for (uint8_t ic = 0; ic < BmsConfig::TOTAL_IC; ++ic) {
        ...
        if (ic_min < pack_min) {
            pack_min      = ic_min;
            pack_min_ic   = ic;
            pack_min_cell = ic_min_c;
        }
        if (ic_max > pack_max) {
            pack_max      = ic_max;
            pack_max_ic   = ic;
            pack_max_cell = ic_max_c;
        }
        ...
        if (ic_t_min != INT16_MAX && ic_t_min < pack_t_min) {
            pack_t_min     = ic_t_min;
            pack_t_min_ic  = ic;
            pack_t_min_ntc = ic_t_min_n;
        }
        if (ic_t_max != INT16_MIN && ic_t_max > pack_t_max) {
            pack_t_max     = ic_t_max;
            pack_t_max_ic  = ic;
            pack_t_max_ntc = ic_t_max_n;
        }
        ...
        Serial.print(F("("));
        Serial.print(ic_min * BmsConfig::CODE_TO_VOLT, 3);
        Serial.print(F(") | "));
        ...
        Serial.print(F("("));
        Serial.print(ic_max * BmsConfig::CODE_TO_VOLT, 3);
        Serial.print(F(") | "));

        if (ic_t_min != INT16_MAX) {
            Serial.print(F("T"));
            Serial.print(ic_t_min_n + 1);
            Serial.print(F("("));
            printTemperatureValue(ic_t_min);
            Serial.print(F(") | "));
        } else {
            Serial.print(F("ERR     | "));
        }
        ...
        if (ic_t_max != INT16_MIN) {
            Serial.print(F("T"));
            Serial.print(ic_t_max_n + 1);
            Serial.print(F("("));
            printTemperatureValue(ic_t_max);
            Serial.print(F(") | "));
        } else {
            Serial.print(F("ERR     | "));
        }
    ...
    Serial.println(F("------------------------------------------------------------------------------------------"));
    Serial.print(F(" Pack V_Min: "));
    Serial.print(pack_min * BmsConfig::CODE_TO_VOLT, 4);
    Serial.print(F("V (IC"));
    Serial.print(pack_min_ic + 1);
    Serial.print(F(" C"));
    if (pack_min_cell + 1 < 10) Serial.print(F("0"));
    Serial.print(pack_min_cell + 1);
    Serial.print(F(")   |   Pack V_Max: "));
    Serial.print(pack_max * BmsConfig::CODE_TO_VOLT, 4);
    Serial.print(F("V (IC"));
    Serial.print(pack_max_ic + 1);
    Serial.print(F(" C"));
    if (pack_max_cell + 1 < 10) Serial.print(F("0"));
    Serial.print(pack_max_cell + 1);
    Serial.print(F(")   |   Delta: "));
    Serial.print((pack_max - pack_min) * BmsConfig::CODE_TO_VOLT, 4);
    Serial.println(F("V"));
    ...
    if (pack_t_min != INT16_MAX) {
        ...
        Serial.print(F(" (IC"));
        Serial.print(pack_t_min_ic + 1);
        Serial.print(F(" T"));
        Serial.print(pack_t_min_ntc + 1);
        Serial.print(F(")"));
    } else {
        Serial.print(F("ERR"));
    }
    ...
    if (pack_t_max != INT16_MIN) {
        ...
        Serial.print(F(" (IC"));
        Serial.print(pack_t_max_ic + 1);
        Serial.print(F(" T"));
        Serial.print(pack_t_max_ntc + 1);
        Serial.print(F(")"));
    } else {
        Serial.print(F("ERR"));
    }
